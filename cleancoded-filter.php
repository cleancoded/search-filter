<?php
/**
 * Plugin Name:     Cleancoded Filter
 * Plugin Slug:     Cleancoded-Search-Filter
 * Plugin Nick:     cleancoded
 * Plugin URI:     https://github.com/cleancoded/search-filter
 * Description:     Search and Filtering system for Pages, Posts, Categories, Tags and Taxonomies.
 * Author:          cleancoded
 * Author URI:      http://cleancoded.com
 * Text Domain:     Cleancoded-Search-Filter
 * Version:         1.0
 */
...
*/
if (!defined('cleancoded_VERSION_NUM'))
    define('cleancoded_VERSION_NUM', '1.0');

if (!defined('cleancoded_THEME_DIR'))
    define('cleancoded_THEME_DIR', ABSPATH . 'wp-content/themes/' . get_template());

if (!defined('cleancoded_PLUGIN_NAME'))
    define('cleancoded_PLUGIN_NAME', trim(dirname(plugin_basename(__FILE__)), '/'));

if (!defined('cleancoded_PLUGIN_DIR'))
    define('cleancoded_PLUGIN_DIR', WP_PLUGIN_DIR . '/' . cleancoded_PLUGIN_NAME);

if (!defined('cleancoded_PLUGIN_URL'))
    define('cleancoded_PLUGIN_URL', WP_PLUGIN_URL . '/' . cleancoded_PLUGIN_NAME);

if (!defined('cleancoded_BASENAME'))
    define('cleancoded_BASENAME', plugin_basename(__FILE__));

if (!defined('cleancoded_VERSION_KEY'))
    define('cleancoded_VERSION_KEY', 'cleancoded_version');
...
add_option(cleancoded_VERSION_KEY, cleancoded_VERSION_NUM);

/*
* Set up Plugin Globals
*/
if ( ! class_exists( 'cleancoded' ) )
{
	class cleancoded
	{
		...
		public function __construct()
		{
			...
			add_shortcode('cleancoded', array($this, 'shortcode'));
		...
		{
			wp_enqueue_style( 'cleancoded', cleancoded_PLUGIN_URL . '/style.css', false, 1.0, 'all' );
		}
		public function of_enqueue_admin_ss($hook)
		{
			if( 'toplevel_page_cleancoded-settings' == $hook )
			{
				wp_enqueue_style( 'of_syntax_style', cleancoded_PLUGIN_URL.'/admin/github.css', false, 1.0, 'all' ); //more highlight styles http://softwaremaniacs.org/media/soft/highlight/test.html
				wp_enqueue_style( 'of_style', cleancoded_PLUGIN_URL.'/admin/style.css', false, 1.0, 'all' );
				//wp_enqueue_script( 'of_syntax_script', cleancoded_PLUGIN_URL.'/admin/syntax.highlight.min.js' );
			}
		}
		...
		*/
		...
		public function get_search_filter_form($submitlabel, $search_placeholder, $fields, $types, $labels, $hierarchical, $hide_empty, $show_count, $post_types, $order_by, $order_dir, $operators, $all_items_labels, $empty_search_url, $add_search_param, $class)
		{
			...
			$returnvar .= '
				<form action="" method="post" class="cleancoded'.$addclass.'">
					<div>';
...
// classes
require_once(cleancoded_PLUGIN_DIR."/of-list-table.php");
require_once(cleancoded_PLUGIN_DIR."/of-taxonomy-walker.php");

// admin screens & plugin mods
require_once(cleancoded_PLUGIN_DIR."/of-admin.php");
